...
# --- 1.2 Check Keycloak Realm Export ---
if [ ! -f "$PROJECT_DIR/realm-export.json" ]; then
  echo "⚠️  Cảnh báo: Không tìm thấy realm-export.json."
  echo "    Identity Service cần file này để cấu hình Keycloak tự động."
else
  echo "ℹ️  realm-export.json found, skipping"
fi
